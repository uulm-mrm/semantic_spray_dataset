...
def visualize_pts_labels_2D(
    ...
    plt.axis("equal")
    plt.tight_layout()
    title_name = "img/det/" + title + ".png"
    plt.savefig(title_name)
    print("Saved:", title_name)
    plt.close()
...
def visualize_pts(
    ...
    if show_intensity:
        ...
        G = mlab.points3d(
            pts[:, 0], pts[:, 1], pts[:, 2], mode="point", colormap="gnuplot", scale_factor=1, figure=fig
        )
    if draw_origin:
...
def draw_corners3d(corners3d, fig, color=(1, 1, 1), line_width=2, cls=None, tag="", max_num=128, tube_radius=None):
    """
    ...
    # # shuffle list:
    # random.shuffle(corners3d)

    num = min(max_num, len(corners3d))
    print("Box Plotting : ", num, "/ ", len(corners3d))
    for n in range(num):
        b = corners3d[n]  # (8, 3)
        ...
        if cls is not None:
            if isinstance(cls, np.ndarray):
                mlab.text3d(
                    b[6, 0], b[6, 1], b[6, 2], "%.2f" % cls[n], scale=(0.3, 0.3, 0.3), color=color, figure=fig
                )
            else:
                mlab.text3d(
                    b[6, 0], b[6, 1], b[6, 2], "%s" % cls[n], scale=(0.3, 0.3, 0.3), color=color, figure=fig
                )
...
def draw_semantic_scene(
    ...
    max_classes=4,
    title="Title",
    save_path="img/nightly_debug.png",
    save=False,
    max_num=128,
):
    if not isinstance(points, np.ndarray):
    ...
    if ref_labels is not None and not isinstance(ref_labels, np.ndarray):
        ref_labels = ref_labels.cpu().numpy()
    if pts_labels is not None and not isinstance(pts_labels, np.ndarray):
        pts_labels = pts_labels.cpu().numpy()

    fig = visualize_semantic_pts(points, pts_labels, max_classes=max_classes, title=title)

    if gt_boxes is not None:
        corners3d = boxes_to_corners_3d(gt_boxes)
        fig = draw_corners3d(corners3d, fig=fig, color=(0, 0, 1), max_num=max_num)

    if ref_boxes is not None and len(ref_boxes) > 0:
        ref_corners3d = boxes_to_corners_3d(ref_boxes)
        if ref_labels is None:
            fig = draw_corners3d(ref_corners3d, fig=fig, color=(0, 1, 0), cls=ref_scores, max_num=max_num)
        else:
            for k in range(ref_labels.min(), ref_labels.max() + 1):
                cur_color = tuple(box_colormap[k % len(box_colormap)])
                mask = ref_labels == k
                fig = draw_corners3d(
                    ref_corners3d[mask], fig=fig, color=cur_color, cls=ref_scores[mask], max_num=max_num
                )
    mlab.view(azimuth=-179, elevation=54.0, distance=84.0, roll=90.0)
    if save:
...
def visualize_semantic_pts(
    pts,
    labels,
    fig=None,
    bgcolor=(0.870, 0.898, 0.937),
    fgcolor=(1.0, 1.0, 1.0),
    size=(600, 600),
    draw_origin=True,
    max_classes=4,
    ...
    # -----------------------------------
    bgcolor = (0.870, 0.898, 0.937)
    color_foreground = (0.0, 0.0, 0.0)
    color_car = (0.427, 0.639, 0.992)
    color_ped = (0.843, 0.635, 0)
    color_cycl = (0.490, 0.211, 0.725)
    color_gas = (0.827, 0.286, 0.286)

    if fig is None:
        fig = mlab.figure(figure=title, bgcolor=bgcolor, engine=None, size=size)

    scale = [0.07, 0.15, 0.15, 0.15, 0.15]
    colors = [color_foreground, color_car, color_ped, color_cycl, color_gas]

    if max_classes == 2:
        colors = [color_foreground, color_gas]
